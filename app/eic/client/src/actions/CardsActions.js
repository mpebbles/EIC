...
export function loadCardsData() {
  ...
  if (userType === "Student") {
    axios({
      method: "get",
      url: "http://localhost:3000/api/get_buddy/",
    ...
    axios({
      method: "get",
      url: "http://localhost:3000/api/get_student/",
      headers: { Authorization: `Bearer ${token}` }
    }).then(res_2 => {
...
export function deleteConnection(email) {
  ...
  try {
    axios({
      method: "get",
      url: "http://localhost:3000/api/get_user_type/",
      headers: { Authorization: `Bearer ${token}` }
    }).then(res => {
      if (res.data === "Student") {
        axios({
          method: "get",
          url: "http://localhost:3000/api/reject_buddy/" + email,
          headers: { Authorization: `Bearer ${token}` }
        }).then(res_1 => {
          if (
        ...
        axios({
          method: "get",
          url: "http://localhost:3000/api/reject_student/" + email,
          headers: { Authorization: `Bearer ${token}` }
        }).then(res_2 => {
          if (
            window.confirm("Are you sure you want to delete this connection?")
