...
describe("/POST create new contract", () => {
  it("it should POST a new contract", done => {
    ...
    googleUser.findOne({contact: 'hycheng@ucsc.edu'},function(err, buddy){
      ...
      .send(contractParam)
      .end((err, res) => {
      	contract.findOne({title: contractParam.title}, function(err, contract){
...
describe("/POST create new company user", () => {
  it("it should create a new company user", done => {
    ...
    googleUser.findOne({contact: 'hycheng@ucsc.edu'},function(err, testUser){
    	 chai.request(app)
      ...
      .auth('user',testUser)
      .end((err, res) => {
      	company.findOne({copmany_name:'MICROSOFT'}, function(err, company){
      		console.log(company);
